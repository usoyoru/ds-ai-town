...
export const LLM_CONFIG = {
   ...
   */
  ollama: true,
  url: 'https://110001001vde958fah7acd249a8910a6355d6789e08v7srv1vcl4.serverless.datastone.cn/',
  chatModel: 'deepseek-r1:1.5b' as const,
...
export async function fetchEmbeddingBatch(texts: string[]) {
  ...
  const {
    ...
    console.log('start embedding:', texts.map((text) => text.replace(/\n/g, ' ')));
    const result = await fetch(apiUrl('/v1/embeddings'), {
      ...
      body: JSON.stringify({
        ...
        prompt: texts.map((text) => text.replace(/\n/g, ' ')),
      }),
    });
    if (!result.ok) {
      throw {
        ...
        try {
          ...
          if (json.choices[0].delta.content) {
            if (json.choices[0].delta.content.startsWith('<think>')) {
              yield json.choices[0].delta.content.split('</think>')[1];
            } else {
...
export async function ollamaFetchEmbedding(text: string) {
  const { result } = await retryWithBackoff(async () => {
    ...
    console.log('Embedding response:', resp)
    if (resp.status === 404) {
